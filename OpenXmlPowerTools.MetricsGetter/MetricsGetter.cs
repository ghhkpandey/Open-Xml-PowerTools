...
namespace OpenXmlPowerTools
{
    ...
    public class MetricsGetter
    {
        ...
        // Delegates to DrawingTextMetrics (in FormattingAssembler) to avoid an upward circular dependency.
        public static int GetTextWidth(FontFamily ff, FontStyle fs, decimal sz, string text)
            => DrawingTextMetrics.GetTextWidth(ff, fs, sz, text);
        ...
        private static XElement RetrieveContentTypeList(OpenXmlPackage oxPkg)
        {
            ...
            var nonRelationshipParts = oxPkg.GetAllParts().Cast<OpenXmlPart>().Where(p => p.ContentType != "application/vnd.openxmlformats-package.relationships+xml");
