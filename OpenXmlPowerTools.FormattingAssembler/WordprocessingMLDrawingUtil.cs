...
{
    // System.Drawing-dependent helpers. Hosted in FormattingAssembler so that
    // FormattingAssembler.CalcWidthOfRunInTwips does not require an upward reference to MetricsGetter.
    public static class WordprocessingMLDrawingUtil
    {
        ...
        public static int CalcWidthOfRunInTwips(XElement r)
        {
            ...
            var bold = WordprocessingMLUtil.GetBoolProp(rPr, W.b) || WordprocessingMLUtil.GetBoolProp(rPr, W.bCs);
            var italic = WordprocessingMLUtil.GetBoolProp(rPr, W.i) || WordprocessingMLUtil.GetBoolProp(rPr, W.iCs);
            if (bold && !italic)
                fs = FontStyle.Bold;
            if (italic && !bold)
                fs = FontStyle.Italic;
            ...
            var w = DrawingTextMetrics.GetTextWidth(ff, fs, sz, runText);
